...
def getdata(i):
    ...
    if len(image_id) < 6:
        for i in range(6-len(image_id)):
            image_id = "0" + image_id
    filename = "./data/val2014/COCO_val2014_000000" + image_id + ".jpg"
    caption = []
...
out = out.view(-1, D)
outword = word_decoding(out)
print outword.size()
m, mi = torch.max(outword,1, keepdim=True)
mi = mi.view(N,T)
ind = mi.data.numpy()
print ind
for i in ind[0]:
